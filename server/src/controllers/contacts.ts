...
const searchContacts =async (req:any,res:any)=>{
try {
    const {searchTerms,email}:{searchTerms:string,email:string} = req.body
...
const findAllContacts = async(req:any,res:any)=>{
    try {
...
const getContactsForChannel = async (req:any,res:any)=>{
    try {
        const {email}= req.body
        console.log(email)
        const user = await prisma.userSchema.findUnique({
